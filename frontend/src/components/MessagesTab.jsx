...
const MessagesTab = ({ isAuthenticated, onLoginClick, onSignupClick, language = 'EN' }) => {
    ...
    useEffect(() => {
        const fetchMessages = async () => {
            ...
            try {
                ...
                // Each section is now { items: [...], total, has_more }
                const allMessages = [];
                if (inboxData.inbox?.items) {
                    allMessages.push(...inboxData.inbox.items.map(msg => ({ ...msg, status: 'inbox' })));
                }
                if (inboxData.public?.items) {
                    allMessages.push(...inboxData.public.items.map(msg => ({ ...msg, status: 'public' })));
                }
                if (inboxData.favorite?.items) {
                    allMessages.push(...inboxData.favorite.items.map(msg => ({ ...msg, status: 'favorite' })));
                }
    ...
    if (!isAuthenticated) {
        return (
            <div className={`messages-tab messages-guest ${isRTL ? 'rtl' : ''}`}>

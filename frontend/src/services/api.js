...
// This is the correct setting when using Kong/Caddy as API gateway
...
export const authAPI = {
  ...
  changePassword: async (oldAnswer, newPhrase, newAnswer) => {
  ...
  googleLink: async (credential) => {
    return apiRequest('/api/auth/google/link', {
      method: 'POST',
      body: JSON.stringify({ credential }),
    });
  },

  googleResetSecurity: async (credential, newPhrase, newAnswer) => {
    const data = await apiRequest('/api/auth/google/reset-security', {
      method: 'POST',
      body: JSON.stringify({
        credential,
        new_phrase: newPhrase,
...
export const messagesAPI = {
  getMessages: async (limit = 20, offset = 0) => {
    return apiRequest(`/api/messages/?limit=${limit}&offset=${offset}`);
  },

  getInbox: async (limit = 20, offset = 0) => {
    // Fetch messages grouped by status with pagination
    return apiRequest(`/api/messages/inbox?limit=${limit}&offset=${offset}`);
  },
...
export const linksAPI = {
  ...
  getLinkMessages: async (privateId, limit = 20, offset = 0) => {
    return apiRequest(`/api/links/${privateId}/messages?limit=${limit}&offset=${offset}`, {

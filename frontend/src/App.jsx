  ...
  return (
    <div>
      ...
      <div className="link-page-close">
        <button onClick={() => navigate('/links')} className="btn secondary">
          ← Back To Links
        </button>
      </div>
    </div>
...
function App() {
  ...
  const ProfileRouteWrapper = ({ isAuthenticated, currentUser }) => {
    ...
    const isValidUsername = /^[a-zA-Z0-9_]{3,50}$/.test(userId || '');
    const isGuest = userId === 'guest';
    const isMe = userId === 'me';
    
    // If trying to access own profile as public profile, redirect to /profile/me
    if (isAuthenticated && currentUser && (userId === String(currentUser.id) || userId === currentUser.username)) {
      return <Navigate to="/profile/me" replace />;
    }

    // Logged-in users should never see guest profile
    if (isAuthenticated && isGuest) {
      return <Navigate to="/profile/me" replace />;
    }

    // Normalize /profile/me to the private route
    if (isMe) {
      return <Navigate to="/profile/me" replace />;
    }

    // Guard invalid profile ids - accept UUID or username
    if (!isGuest && !isValidUUID && !isValidUsername) {
      return <Navigate to={isAuthenticated ? '/home' : '/links'} replace />;
    }
          ...
          <Route
            ...
            element={
              <SettingsPage
                ...
                onLogout={handleLogout}
                onLanguageChange={handleLanguageChange}
                onUserUpdate={refreshCurrentUser}
              />
            }
          />
          <Route
